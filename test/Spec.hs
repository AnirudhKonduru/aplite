...
main = hspec $ do
  ...
  describe "parse an expression" $ do
    ...
    it "parses a monadic expression with mixed types" $ do parse Parser.expressionParser "" "+ 1 2 (3 4.5 (5 7 8) (- 5 2)) 9" `shouldBe` Right (EMonadic (MSym '+') (EArray [EValue (Scalar (IntVal 1)),EValue (Scalar (IntVal 2)),EArray [EValue (Scalar (IntVal 3)),EValue (Scalar (FloatVal 4.5)),EArray [EValue (Scalar (IntVal 5)),EValue (Scalar (IntVal 7)),EValue (Scalar (IntVal 8))],EMonadic (MSym '-') (EArray [EValue (Scalar (IntVal 5)),EValue (Scalar (IntVal 2))])],EValue (Scalar (IntVal 9))]))
    ...
    it "parses a dyadic expression #2" $ do parse Parser.expressionParser "" "1 5 6 + 2 3 8" `shouldBe` Right (EDyadic (EValue (Array [3] [Scalar (IntVal 1),Scalar (IntVal 5),Scalar (IntVal 6)])) (DSym '+') (EValue (Array [3] [Scalar (IntVal 2),Scalar (IntVal 3),Scalar (IntVal 8)])))
